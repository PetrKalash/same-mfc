...
// SameGameTest.cpp: определяет поведение классов для приложения.
//
...
BEGIN_MESSAGE_MAP(CSameGameTestApp, CWinApp)
	ON_COMMAND(ID_APP_ABOUT, &CSameGameTestApp::OnAppAbout)
...
// Создание CSameGameTestApp

CSameGameTestApp::CSameGameTestApp() noexcept
{

	// TODO: замените ниже строку идентификатора приложения строкой уникального идентификатора; рекомендуемый
	// формат для строки: ИмяКомпании.ИмяПродукта.СубПродукт.СведенияОВерсии
	SetAppID(_T("SameGameTest.AppID.NoVersion"));

	// TODO: добавьте код создания,
	// Размещает весь важный код инициализации в InitInstance
}

// Единственный объект CSameGameTestApp

CSameGameTestApp theApp;


// Инициализация CSameGameTestApp

BOOL CSameGameTestApp::InitInstance()
{
	...
	pDocTemplate = new CSingleDocTemplate(
		IDR_MAINFRAME,
		RUNTIME_CLASS(CSameGameTestDoc),
		RUNTIME_CLASS(CMainFrame),       // основное окно рамки SDI
		RUNTIME_CLASS(CSameGameTestView));
	if (!pDocTemplate)
		return FALSE;
...
void CSameGameTestApp::OnAppAbout()
{
	CAboutDlg aboutDlg;
	aboutDlg.DoModal();
}
