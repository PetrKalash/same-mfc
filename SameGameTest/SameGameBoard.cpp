...
void CSameGameBoard::compact_board()
{
	...
	// Смещение ячеек влево - находим пустой столбец по всем строкам (у него должна быть гарантированно пустая первая ячейка)
	for (size_t cols{}; cols < m_cols; ++cols)
	{
		...
		for (size_t rows{}; rows < m_rows; ++rows)
		{
			...
			// Проверка, что у столбца все блоки пустые
